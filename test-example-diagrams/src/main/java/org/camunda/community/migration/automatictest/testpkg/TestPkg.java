...
public class TestPkg {
  ...
  /**
   * detect a TestPck
   * A test package must have different files, starting with the same name
   * XXX.bpmn (or XXX_C7.bpmn) is the Camunda 7 process
   * XXX.Automator.json is the Automator scenario
   * XXX-C8.bpmn is the Camunda 8 process
   * The detection is based on the Automator file.
   *
   * @param file
   * @return
   */
  public static TestPkg createForFile(File file) throws Exception {
    if (!file.getName().endsWith("Automator.json"))
      return null;
  ...
  public boolean isComplete() {
    return automator!=null && automator.exists()
        && camunda7!=null && camunda7.exists()
        && camunda8!=null && camunda8.exists();
  }
  ...
  public String getIncompleteStatus() {
    ...
    if (automator==null || !automator.exists())
      report.append("Automator scenario is missing;");
    if (camunda7 == null || !camunda7.exists())
      report.append("Camunda 7 process is missing;");
    if (camunda8==null || !camunda8.exists())
      report.append("Camunda 8 process is missing;");
    return report.toString();
  }

  public Scenario getScenario() throws AutomatorException {
    if (automator.exists())
      return Scenario.createFromFile(automator);
    return null;
  }
