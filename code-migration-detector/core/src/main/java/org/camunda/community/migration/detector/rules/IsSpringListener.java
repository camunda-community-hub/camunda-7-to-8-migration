...
public class IsSpringListener<T> extends DescribedPredicate<JavaMethod> {
	...
	@Override public boolean test(JavaMethod javaMethod) {
		return javaMethod.isAnnotatedWith(EventListener.class) && javaMethod.getRawParameterTypes().stream().anyMatch(
				(clazz) -> eventType.isAssignableFrom(clazz.reflect()));
	}
}
